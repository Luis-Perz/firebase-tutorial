        ...
        <div className="field">
          ...
          <div className="drop-zone" id="drop-zone" onClick={() => document.getElementById('pdf-input').click()}>
            <div className="drop-icon">
                <p className="drop-label" id="drop-label">{file ? file.name : 'Drop PDF here or click to browse'}</p>
                <p className="drop-hint">PDF only — max 10MB</p>
                <p className="drop-file-name" id="file-name"></p>
            </div>
          </div>
          <input type="file" id="pdf-input" accept=".pdf" style={{ display: 'none' }} onChange={e => setFile(e.target.files[0])} />
        </div>
